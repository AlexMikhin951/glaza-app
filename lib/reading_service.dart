...
class ReadingService {
  ...
  /// Облачный OCR fallback (Yandex Vision), если ML Kit пуст.
  Future<String?> Function(Uint8List jpeg)? cloudOcrFallback;
  ...
  String _lastText = '';
  int _lastLiveOcrMs = 0;
  String _lastLiveSnippet = '';
  ...
  /// Мгновенный OCR потока: только новый текст, без повторов (чтение 2.0).
  Future<void> liveOcrTick(Uint8List jpegBytes) async {
    if (_isScanning) return;
    final now = DateTime.now().millisecondsSinceEpoch;
    if (now - _lastLiveOcrMs < 1000) return;
    _lastLiveOcrMs = now;

    try {
      final text = await _recognizeText(jpegBytes);
      final snippet = _summarizeText(text);
      if (snippet.isEmpty || snippet == _lastLiveSnippet) return;
      if (text.trim().length < 8) return;
      _lastLiveSnippet = snippet;
      _lastText = text;
      await _enqueue(snippet, priority: 0);
    } catch (_) {}
  }
  ...
  Future<String> _recognizeText(Uint8List jpegBytes) async {
    try {
      ...
      final local = result.text.trim();
      if (local.isNotEmpty) return local;
    } catch (e) {
      debugPrint('ReadingService ML Kit error: $e');
    }

    final cloud = cloudOcrFallback;
    if (cloud != null) {
      final remote = await cloud(jpegBytes);
