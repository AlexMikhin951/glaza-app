// lib/main.dart
import 'package:flutter/material.dart';
import 'package:flutter/services.dart';
import 'package:yandex_maps_mapkit/init.dart' as mapkit_init;
import 'app_theme.dart';
import 'mapkit_api_key.dart';
import 'setup_screen.dart';

void main() async {
  WidgetsFlutterBinding.ensureInitialized();

  // Предпочитаем dart-define, но оставляем локальный ключ как fallback
  // для отладки на подключённом телефоне.
  const mapkitApiKey = String.fromEnvironment('MAPKIT_API_KEY');
  final effectiveMapkitApiKey = mapkitApiKey.isNotEmpty
      ? mapkitApiKey
      : kYandexMapkitApiKey;
  if (effectiveMapkitApiKey.isNotEmpty) {
    await mapkit_init.initMapkit(
      apiKey: effectiveMapkitApiKey,
      locale: 'ru_RU',
    );
  }
  SystemChrome.setEnabledSystemUIMode(SystemUiMode.edgeToEdge);
  SystemChrome.setSystemUIOverlayStyle(
    const SystemUiOverlayStyle(
      statusBarColor: Colors.transparent,
      statusBarIconBrightness: Brightness.light,
      systemNavigationBarColor: AppColors.background,
      systemNavigationBarIconBrightness: Brightness.light,
    ),
  );
  PaintingBinding.instance.imageCache.maximumSize = 2;
  PaintingBinding.instance.imageCache.maximumSizeBytes = 10 * 1024 * 1024;
  runApp(const SmartGlassesApp());
}

class SmartGlassesApp extends StatelessWidget {
  const SmartGlassesApp({super.key});

  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'Smart Glasses AI',
      theme: buildAppTheme(),
      debugShowCheckedModeBanner: false,
      home: const SetupScreen(),
    );
  }
}
